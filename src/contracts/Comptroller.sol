    ...
    ExponentialNoError
{
    ...
    /// @notice Emitted when an address is whitelisted as a recipient of cToken transfers
    event WhitelistedRecipient(address recipient, bool state);
    ...
    function transferAllowed(
        ...
        // Pausing is a very serious situation - we revert to sound the alarms
        if (!transferAllowedWhitelist[dst]) {
    ...
    function _setTransferAllowedWhitelist(address dst, bool state)
        public
        returns (bool)
    {
        require(
        ...
        transferAllowedWhitelist[dst] = state;
        emit WhitelistedRecipient(dst, state);
        return state;
    }
