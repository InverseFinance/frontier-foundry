...
contract ConvexStakingWrapper is ERC20, ReentrancyGuard {
    ...
    function initialize(
        ...
        _tokensymbol = string(
            abi.encodePacked("stk", ERC20(_convexToken).symbol())
        );
        isShutdown = false;
        isInit = true;
        ...
        returns (EarnedData[] memory claimable)
    {
        ...
        for (uint256 i = 0; i < rewardCount; i++) {
            ...
            uint256 bal = IERC20(reward.reward_token).balanceOf(address(this));
            uint256 d_reward = bal.sub(reward.reward_remaining);
            d_reward = d_reward.add(
                IRewardStaking(reward.reward_pool).earned(address(this))
            );
            ...
            if (i == CRV_INDEX) {
                ...
                if (supply > 0) {
                    I =
                        I +
                        IRewardStaking(reward.reward_pool)
                            .earned(address(this))
                            .mul(1e20)
                            .div(supply);
                }
                newlyClaimable = _getDepositedBalance(_account)
                    .mul(I.sub(reward.reward_integral_for[_account]))
                    .div(1e20);
                claimable[CVX_INDEX].amount = CvxMining.ConvertCrvToCvx(
                    newlyClaimable
                );
    ...
    function stake(uint256 _amount, address _to) external {
        ...
        if (_amount > 0) {
            _mint(_to, _amount);
            IERC20(convexToken).safeTransferFrom(
                msg.sender,
                address(this),
                _amount
            );
